...
+ (BOOL)isAccountLockedForUser:(NSString *)username error:(NSError **)lockError {
    ...
    if(lockString != nil && ![lockString isEqualToString:@""]) {
        ...
        *lockError = [PPBaseModel resultCodeToNSError:16 originatingClass:NSStringFromClass([self class]) argument:[NSString stringWithFormat:NSLocalizedString(@"Account is locked until %@", @"Error - Account is locked until {lockTime}"), [NSDateFormatter localizedStringFromDate:[NSDate dateWithTimeIntervalSince1970:(lockTime / 1000)] dateStyle:NSDateFormatterShortStyle timeStyle:NSDateFormatterShortStyle]]];
        if(lockTime > now) {
            return YES;
        }
        else {
