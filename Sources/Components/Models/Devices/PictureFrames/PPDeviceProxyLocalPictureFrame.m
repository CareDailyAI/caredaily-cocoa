//
//  PPDeviceProxyLocalPictureFrame.m
//  Peoplepower
//
...
@interface PPDeviceProxyLocalPictureFrame ()
...
- (PPDevicePictureFrameLocal *)device {
    ...
    if([device isKindOfClass:[PPDevicePictureFrame class]]) {
        return (PPDevicePictureFrameLocal *)[PPDevices localDeviceForLocation:[[PPUserAccounts currentUser] currentLocation] userId:[PPUserAccounts currentUser].userId];
    }
    return nil;
}
...
+ (BOOL)wasProxyOn {
    ...
    NSString *proxyWasOn = [defaults stringForKey:@"proxy.pictureFrame.wasOn"];
...
#endif
    if([self.delegate respondsToSelector:@selector(pictureFrameMustStartPublishing)]) {
        [self.delegate pictureFrameMustStartPublishing];
    }
}
...
- (void)hangUpVideoCall:(NSString *)sessionId {
    ...
    if([self.delegate respondsToSelector:@selector(pictureFrameHungUpVideoCall)]) {
        [self.delegate pictureFrameHungUpVideoCall];
    }
}
...
- (void)stopPublishing {
    ...
    NSLog(@"%s", __PRETTY_FUNCTION__);
#endif
    if([self.delegate pictureFrameIsPublishing]) {
        [self hangUpVideoCall:@""];
        [self.delegate pictureFrameMustStopPublishing];
    }
}
...
//           || [command.name isEqualToString:MODE]
           || [command.name isEqualToString:ALERT_TITLE]
           || [command.name isEqualToString:ALERT_SUBTITLE]
           || [command.name isEqualToString:ALERT_QUESTION_ID]
           || [command.name isEqualToString:ALERT_MESSAGE]
           || [command.name isEqualToString:ALERT_ICON]
           || [command.name isEqualToString:ALERT_TIMESTAMP_MS]
           || [command.name isEqualToString:ALERT_DURATION_MS]
           || [command.name isEqualToString:ALERT_PRIORITY]
           || [command.name isEqualToString:ALERT_STATUS]
           || [command.name isEqualToString:NOTIFICATION]
           ) {
...
- (void)websocket:(SRWebSocket*)webSocket didReceiveMessage:(id)message {
    ...
    if(totalViewers == 0) {
        if([self.delegate respondsToSelector:@selector(pictureFrameIsPublishing)]) {
            if([self.delegate pictureFrameIsPublishing]) {
                [self stopPublishing];
            }
        }
