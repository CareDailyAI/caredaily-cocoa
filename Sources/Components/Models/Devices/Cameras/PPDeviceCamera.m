    ...
    if(self) {
        ...
        for(PPDeviceParameter *defaultParam in [PPDeviceCamera defaultParameters]) {
            ...
            if(!found) {
                ...
                if([defaultParam.name isEqualToString:MOTION_COUNTDOWN_TIME]) {
                    _motionCountDownTime = (PPDeviceParametersMotionCountDownTime)defaultParam.value.integerValue;
                }
                else if([defaultParam.name isEqualToString:RECORD_SECONDS]) {
                    _recordSeconds = (PPDeviceParametersRecordSeconds)defaultParam.value.integerValue;
                }
                else if([defaultParam.name isEqualToString:RAPID_MOTION_STATUS]) {
                    _rapidMotionStatus = (PPDeviceParametersRapidMotionStatus)defaultParam.value.integerValue;
                }
...
- (id)initWithDeviceId:(NSString *)deviceId proxyId:(NSString *)proxyId name:(NSString *)name connected:(PPDeviceConnected)connected restricted:(PPDeviceRestricted)restricted shared:(PPDeviceShared)shared newDevice:(PPDeviceNewDevice)newDevice goalId:(PPDeviceTypeGoalId)goalId typeId:(PPDeviceTypeId)typeId category:(PPDeviceTypeCategory)category typeAttributes:(RLMArray *)typeAttributes locationId:(PPLocationId)locationId startDate:(NSDate *)startDate lastDataReceivedDate:(NSDate *)lastDataReceivedDate lastMeasureDate:(NSDate *)lastMeasureDate lastConnectedDate:(NSDate *)lastConnectedDate parameters:(RLMArray *)parameters properties:(RLMArray *)properties icon:(NSString *)icon spaces:(RLMArray *)spaces modelId:(NSString *)modelId __attribute__((deprecated)) {
    ...
    return [super initWithDeviceId:deviceId proxyId:proxyId name:name connected:connected restricted:restricted shared:shared newDevice:newDevice goalId:goalId typeId:typeId category:category typeAttributes:typeAttributes locationId:locationId startDate:startDate lastDataReceivedDate:lastDataReceivedDate lastMeasureDate:lastMeasureDate lastConnectedDate:lastConnectedDate parameters:parameters properties:properties icon:icon spaces:spaces modelId:modelId userId:PPUserIdNone];
}

+ (NSMutableArray *)defaultParameters {
    NSMutableArray *defaultParameters = [[NSMutableArray alloc] initWithCapacity:0];
    [defaultParameters addObject:[[PPDeviceParameter alloc] initWithName:MOTION_COUNTDOWN_TIME index:nil value:@(PPDeviceParametersMotionCountDownTimeDefault).stringValue lastUpdateDate:[NSDate date]]];
    [defaultParameters addObject:[[PPDeviceParameter alloc] initWithName:RECORD_SECONDS index:nil value:@(PPDeviceParametersRecordSecondsDefault).stringValue lastUpdateDate:[NSDate date]]];
    [defaultParameters addObject:[[PPDeviceParameter alloc] initWithName:RAPID_MOTION_STATUS index:nil value:@(PPDeviceParametersRapidMotionStatusDefault).stringValue lastUpdateDate:[NSDate date]]];
...
+ (NSArray *)kvoObservers {
    return @[@"selectedCamera",
             ...
             @"robotVantageTimers",
             @"robotVantageNames",
             @"robotVantageSequence",
             @"robotVantageConfigurationStatus",
             @"robotVantageMoveToIndex",
             @"robotOrientation",
             @"twitterReminder",
             @"twitterAutoShare",
             @"twitterDescription",
...
- (void)setParameter:(NSString *)paramName value:(NSString *)paramValue index:(NSString *)paramIndex lastUpdateDate:(NSDate *)paramLastUpdateDate {
    ...
    // Twitter
        ...
        if(self.twitterAutoShare != paramValue.integerValue) {
            self.twitterAutoShare = paramValue.integerValue;
        }
    }
...
#warning This is breaking Realm
//#ifdef DEBUG
