...
+ (PPRuleComponentState *)initScheduleStateComponent:(PPRuleComponentStateType)stateType {
    PPRuleComponentParameter *param1 = [[PPRuleComponentParameter alloc] initWithName:RULE_COMPONENT_STATE_SCHEDULE_COMPONENT_PAMAMETER_NAME_1 category:PPRuleComponentParameterCategoryCronExpression optional:PPRuleComponentParameterOptionalFalse desc:NSLocalizedStringWithDefaultValue(@"rule.component.state.schedule.start", nil, [PPBaseModel bundle], @"What is the start time, and on which days?", @"Label - What is the start time, and on which days?") values:nil selectorName:nil value:nil minValue:PPRuleComponentParameterMinValueNone maxValue:PPRuleComponentParameterMaxValueNone valueType:PPRuleComponentParameterInputTypeNone unit:nil];
    
    PPRuleComponentParameter *param2 = [[PPRuleComponentParameter alloc] initWithName:RULE_COMPONENT_STATE_SCHEDULE_COMPONENT_PAMAMETER_NAME_2 category:PPRuleComponentParameterCategoryCronExpression optional:PPRuleComponentParameterOptionalFalse desc:NSLocalizedStringWithDefaultValue(@"rule.component.state.schedule.stop", nil, [PPBaseModel bundle], @"What time should it end each day?", @"Label - What time should it end each day?") values:nil selectorName:nil value:nil minValue:PPRuleComponentParameterMinValueNone maxValue:PPRuleComponentParameterMaxValueNone valueType:PPRuleComponentParameterInputTypeNone unit:nil];
    
    PPRuleComponentState *state = [[PPRuleComponentState alloc] initWithId:PPRuleComponentIdNone
    name:RULE_COMPONENT_STATE_SCHEDULE_COMPONENT_NAME stateType:stateType displayType:PPRuleComponentDisplayTypeStateGeneralCondition
    desc:NSLocalizedStringWithDefaultValue(@"rule.component.state.schedule", nil, [PPBaseModel bundle], @"$time1 and $time2", @"Label - $time1 and $time2") past:NSLocalizedStringWithDefaultValue(@"rule.component.state.schedule.past", nil, [PPBaseModel bundle], @"between $time1 and $time2", @"Label - between $time1 and $time2") timezone:PPRuleComponentTimezoneTrue functionGroup:nil parameters:(RLMArray *)@[param1, param2] serviceName:nil];
    return state;
}
